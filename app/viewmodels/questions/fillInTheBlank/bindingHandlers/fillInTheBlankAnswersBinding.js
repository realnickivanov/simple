define(['durandal/composition', 'translation'], function (composition, translation) {
    ...
    ko.bindingHandlers.fillInTheBlankAnswers = {
        init: function (element, valueAccessor) {
            ...
            $(blankInputSelector, $element).each(function (index, blankItem) {
                ...
                if ($blankItem.is('input')) {
                    createInput($blankItem, blankValue);
                } else if ($blankItem.is('select')) {
                    createDropdown($blankItem, blankValue, element, disabled);
                }
            });
        },
        update: function (element, valueAccessor) {
            ...
            $(blankInputSelector, $element).each(function (index, blankItem) {
                ...
                $blankItem.attr('tabindex', disabled ? -1 : 0);

                if ($blankItem.is('input')) {
                    $blankItem.val(blankValue.value);
                } else if ($blankItem.is('div')) {
                    var $currentItem = $('.' + cssClasses.currentSelectedItemText, $blankItem);
                    if (!disabled) {
    ...
    function createInput($blankItem, blankValue) {
    ...
    function createDropdown($element, blankValue, blanksHolder, isAnswered) {
        var
        ...
        function changeCurrentSelectedItem() {
            $currentSelectedItemText.text($selectedItem.text()).data('index', $listItems.index($selectedItem)).removeClass(cssClasses.currentSelectedItemTextPlaceholder);
        }
        ...
        $currentSelectedItemText = $('<span class="' + cssClasses.currentSelectedItemText + '"></span>');
